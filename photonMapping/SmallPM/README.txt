/*********************************************************************************
Copyright (C) 2016 Adrian Jarabo (ajarabo@unizar.es)
Copyright (C) 2016 Diego Gutierrez (diegog@unizar.es)
All rights reserved.
...
1-	The following code has been though as an educational piece of software to teach
	...
	As such, it is designed for Lambertian and delta (perfect transmissive and specular)
	BSDFs. The reason is to hide as much as possible the complexities related with 
	particle tracing and Monte Carlo integration in general. However, you can add Phong
	BRDFs, so you can compute the indirect illumination over this type of surfaces using
...
4-	In order to access the info of the material at a given intersection 'it' you
	can use the function 'Intersection::intersected()->get_material()', which will
	return a pointer to a BSDF (see SmallPT/SmallRT/include/BSDF.h for details on
	that virtual class). 

5-	This code has been developed and tested in Windows, and it builds on top of a
	...
	in the provided Visual Studio solutions for versions 2013, 2015. These are easy 
	to get, and thus, for the sake of simplicity we recomend using the provided VS 
