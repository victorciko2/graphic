...
int main(int argc, char* argv[])
{
	...
	for( int i=1; i<argc; ++i)
	{
		...
		if( !strcmp("-pm-photons-media",argv[i]) ) {++i; photons_media = atoi(argv[i]); }
	...
	max_shots = (max_shots > (photons_global + photons_caustic + photons_media))? max_shots:(photons_global + photons_caustic + photons_media);
	...
	{	
		cout << "case 3" << endl;
		Object3D* sphere1 = new Sphere(Vector3(0,0.9,0), 0.3, glass);
		w->add_object(sphere1);
	...
	LightSource* ls = new PointLightSource(w, Vector3(0,1.9,0), Vector3(0.7,0.7,0.7));
	w->add_light(ls);
	...
	pm = new PhotonMapping(w, photons_global, photons_caustic, photons_media, max_shots, nb_nearest_photons );
